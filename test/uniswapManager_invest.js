...
describe("Invest Manager with Uniswap", function () {
  ...
  let wallet; let uniswapFactory; let investManager; let
    token;
  ...
  async function testCreatePool(initialEthLiquidity, initialTokenPrice) {
    const initialTokenAmount = initialEthLiquidity.mul(initialTokenPrice);
  ...
  async function addInvestment(ethLiquidity, tokenPrice, amount, relay = false) {
    const ethBefore = await deployer.provider.getBalance(wallet.contractAddress);
    const tokenBefore = await token.balanceOf(wallet.contractAddress);
    const pool = await testCreatePool(ethLiquidity, tokenPrice);
    ...
    const shares = await pool.balanceOf(wallet.contractAddress);
    const ethAfter = await deployer.provider.getBalance(wallet.contractAddress);
    const tokenAfter = await token.balanceOf(wallet.contractAddress);
  ...
  async function removeInvestment(investedAmount, fraction, relay = false) {
    const ethLiquidity = parseEther("0.1");
    const tokenPrice = 3;
    ...
    const result = await addInvestment(ethLiquidity, tokenPrice, investedAmount, 0);
    const sharesBefore = result[1];
    ...
    const sharesAfter = await result[0].balanceOf(wallet.contractAddress);
  ...
  describe("Add investment", () => {
    ...
    for (i = 0; i < 10; i++) {
      const ethLiquidity = parseEther("0.1").add(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER) + 1);
      const tokenPrice = Math.floor(Math.random() * 100) + 1;
      const amount = parseEther("0.001").add(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER) + 1);
  ...
  describe("Remove investment", () => {
    ...
    for (i = 0; i < 10; i++) {
      const investedAmount = parseEther("0.0001").mul(Math.floor(Math.random() * 1000) + 1);
      const fraction = (i + 1) * 1000;
